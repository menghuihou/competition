...
a = []
for i in range(len(container)):
    weekly_damage_container.append(round(container[i] * 0.2))
    ...
    if (i + 1 < len(arm)):
        ...
        if (s > initial_arm):  # 机械臂
            ...
            add_arm.append(num1)
            a = math.ceil(num1 / 10)
            v.append(num1 + math.ceil(num1 / 10))  # 参与训练的操作手数量
            initial_arm = s
            weeks_arm.append(i)
    ...
    #weekly_maintenance_arm.append(initial_arm-arm[103])
...
wb.save('问题二：1-104周的结果数据.xlsx')
wb = load_workbook('问题二：1-104周的结果数据.xlsx')
sheet = wb.active
